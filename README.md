...
- 按 **F11 进入全屏模式**：连 Windows 11 底部的任务栏一起隐藏，整块屏幕都是她。
...
| 按键 | 作用 |
|---|---|
| **F11** | **全屏模式**：隐藏 / 恢复 Windows 底部任务栏（含副屏任务栏）。再按一次就回来 |
| **Ctrl+Alt+D** | 临时关闭 / 恢复壁纸模式（关闭时把游戏窗口还原成普通窗口，方便你正常玩） |

F11 只在游戏运行时生效，不会劫持系统热键；不想要就把它改掉或关掉（见下面的配置）。
退出游戏时任务栏和桌面图标都会自动还原。
...
| `[Fullscreen] Enabled` | true | 是否启用 F11 全屏模式热键 |
| `[Fullscreen] ToggleKey` | `F11` | 全屏模式热键。支持 `F1`~`F24` / `A`~`Z` / `0`~`9`；填 `NONE` 关闭 |
| `[Fullscreen] HideTaskbarOnStart` | false | 启动时就隐藏任务栏（默认靠 F11 手动切） |
| `[Layout] *` | 2560×1600 标定 | 按钮位置常量，换分辨率/改游戏 UI 时可微调 |

## 已知问题 / 注意

- 插件会临时隐藏**桌面图标**，按 F11 后还会隐藏**任务栏**。正常退出游戏时会自动还原；
  但如果是**杀进程**结束的游戏，插件来不及还原，这时运行 `tools\Restore-Desktop.ps1` 即可恢复
  （或者在任务管理器里重启「Windows 资源管理器」）。
- 壁纸状态下游戏窗口**不会抢焦点**，所以 **Alt+F4 关掉的是你当前正在用的那个窗口，不是游戏**。
  想退出游戏请用它自己画面里的退出按钮，或者先按 `Ctrl+Alt+D` 把游戏还原成普通窗口再 Alt+F4。
- 按钮位置常量按 **2560×1600** 标定，其他分辨率按比例缩放；如果和你的游戏 UI 对不齐，
  可以调 `BepInEx\config\com.chilldesktop.plugin.cfg` 里的 `[Layout]` 一组参数。
- 目前只处理**主屏**。
...
**F11 全屏模式**：直接对 `Shell_TrayWnd`（主屏任务栏）和 `Shell_SecondaryTrayWnd`（副屏任务栏）
调 `ShowWindow(SW_HIDE)`；壁纸窗本来就铺满整屏，任务栏一藏底下那条也露出壁纸。
心跳只在「句柄失效（Explorer 重启）」时重新接管，**不会**跟用户按 Win 键唤出任务栏的动作抢。
